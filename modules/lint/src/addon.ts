...
export function generateCode(context: NustackContext): string {
  return [
    ...
    `import { nustackLint as factory } from '@nustackjs/lint/config'`,
    `import withNuxt from './eslint.config.mjs'`,
    '',
    `export const nustackContext = ${JSON.stringify(context, null, 2)}`,
    '',
    '/**',
    ' * Project-aware ESLint config, with `withNuxt()` already wired in. Pass factory',
    ' * options (variant, per-concern toggles, overrides); any extra arguments are',
    ' * forwarded to `withNuxt()` as additional flat configs. Returns the composer, so',
    ' * you can chain `.append()` / `.override()`.',
    ' */',
    'export function nustackLint(options = {}, ...customs) {',
    '  return factory(withNuxt(...customs), { ...options, context: nustackContext })',
    '}',
    '',
